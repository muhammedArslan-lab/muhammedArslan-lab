# 👋 Hi, I'm Muhammad Arslan

🚀 **React Native Developer** passionate about building high-quality, scalable mobile applications with clean UI, solid architecture, and real-world integrations.

I enjoy turning complex requirements into smooth user experiences and writing maintainable code that works reliably in production — not just demos.

---

## 👨‍💻 About Me

- 📱 React Native developer with real-world, production app experience
- ⚛️ Strong with React Hooks, Context API, Redux & React Query
- 🎨 Built pixel-perfect, scalable UI based on Figma designs
- 🔥 Experience integrating REST APIs, Firebase, AWS services & CI/CD pipelines
- 💳 Implemented monetization via In-App Purchases & Subscriptions
- 🧠 Focused on clean code, reusable components & performance
- 🛠 Love debugging complex issues and maintaining long-term app stability
- 🌱 Always learning better architecture, tooling & platform updates

---

## 🚀 Live Apps & Projects

Here are some mobile applications I’ve worked on that are **live on the App Store and Google Play**.  
I’ve also contributed to multiple private and in-progress applications.

### 🏋️‍♂️ FIT MOTIVATE  
A fitness-focused mobile app designed to support gym users and structured fitness programs.

- 📱 **Platform:** Android & iOS  
- 🎨 **Work:** UI development (pixel-perfect), API integration, Stripe Payment Gateway Integration, Real-Time Chat Integration  
- 💳 **Features:** In-App Subscriptions  
- 🛠 **Tech:** React Native, Redux, React Query, REST APIs, Firebase  

👉 **Android:** https://play.google.com/store/apps/details?id=com.fit_motivate  
👉 **iOS:** https://apps.apple.com/pk/app/fitmotivate-hyrox-gym-partner/id6747404305

---

### 🩺 MUNDOSALUD  
A healthcare-related application focused on accessibility and user-friendly experience.

- 📱 **Platform:** Android  
- 🎨 **Work:** UI development, API integration, In App Updates/OTA Updates, Stripe Payment Gateway Integration, Real-Time Chat Integration  
- 🛠 **Tech:** React Native, REST APIs  

👉 **Android:** https://play.google.com/store/apps/details?id=com.app.mundoSalud

---

### 🕌 MUSLIM COUNCIL  
A community-focused application providing announcements, resources, and engagement features.

- 📱 **Platform:** Android & iOS  
- 🎨 **Work:** UI development, API integration, In App Updates/OTA Updates  
- 🔔 **Features:** Push notifications  
- 🛠 **Tech:** React Native, Firebase  

👉 **Android:** https://play.google.com/store/apps/details?id=com.muslimcouncil  
👉 **iOS:** https://apps.apple.com/pk/app/muslim-council-calgary/id6450579624

---

### 📖 VERBA VERSE  
A content-driven reading and engagement application.

- 📱 **Platform:** iOS  
- 🎨 **Work:** UI development, API integration, Stripe Payment Gateway Integration, Real-Time Chat Integration
- 🛠 **Tech:** React Native, REST APIs  

👉 **iOS:** https://apps.apple.com/us/app/verbaverse/id6739344483

---

## 🧠 Core Mobile Integrations & Capabilities

- 💳 **In-App Purchases & Subscriptions** (iOS & Android)
- 📊 **Google AdMob Integration**
- 🔌 **REST API Integration**
- 🔄 **WebSockets / Real-time Chat (1–1 & Group)**
- 📹 **Video Calling Integration**
- 🔔 **Push & Local Notifications**
- 🔐 **Authentication** (Firebase Auth, Google & Apple Sign-In)
- ☁️ **Firebase Services** (Auth, Storage, FCM)
- 💸 **Stripe Payment Integration**
- 🔁 **CI/CD Integration** (automated builds & deployments)
- 🧩 **Third-party SDK & library integration**
- 📱 **App Store & Play Store publishing**
- 🆕 **Upgraded Android apps to comply with Google Play’s 16KB memory page size requirement**
- ⚙️ **Performance optimization, dependency conflict resolution & bug fixing**

---

## 🛠 Tech Stack

### 📱 Mobile
![React Native](https://img.shields.io/badge/React%20Native-black?logo=react)
![Expo](https://img.shields.io/badge/Expo-black?logo=expo)

### ⚛️ Frontend
![JavaScript](https://img.shields.io/badge/JavaScript-black?logo=javascript)
![TypeScript](https://img.shields.io/badge/TypeScript-black?logo=typescript)
![React](https://img.shields.io/badge/React-black?logo=react)
![React Query](https://img.shields.io/badge/React%20Query-black?logo=reactquery)

### 🧠 State & Forms
![Redux](https://img.shields.io/badge/Redux-black?logo=redux)
![Redux Toolkit](https://img.shields.io/badge/Redux%20Toolkit-black?logo=redux)
![React Hook Form](https://img.shields.io/badge/React%20Hook%20Form-black?logo=reacthookform)

### 🌐 Backend & APIs
![Node.js](https://img.shields.io/badge/Node.js-black?logo=node.js)
![Express](https://img.shields.io/badge/Express-black?logo=express)
![REST API](https://img.shields.io/badge/REST%20API-black)

### ☁️ Cloud & Backend Services
![Firebase](https://img.shields.io/badge/Firebase-black?logo=firebase)
![AWS](https://img.shields.io/badge/AWS-black?logo=amazonaws)
![Supabase](https://img.shields.io/badge/Supabase-black?logo=supabase)

### 🗄 Databases
![MongoDB](https://img.shields.io/badge/MongoDB-black?logo=mongodb)
![MySQL](https://img.shields.io/badge/MySQL-black?logo=mysql)

### 🖥 Native & Mobile Languages
![Swift](https://img.shields.io/badge/Swift-black?logo=swift)
![Objective-C](https://img.shields.io/badge/Objective--C-black?logo=apple)
![Java](https://img.shields.io/badge/Java-black?logo=java)
![Kotlin](https://img.shields.io/badge/Kotlin-black?logo=kotlin)

### 🧰 Tools & IDEs
![Visual Studio Code](https://img.shields.io/badge/VS%20Code-black?logo=visualstudiocode)
![Android Studio](https://img.shields.io/badge/Android%20Studio-black?logo=androidstudio)
![Xcode](https://img.shields.io/badge/Xcode-black?logo=xcode)
![Git](https://img.shields.io/badge/Git-black?logo=git)
![GitHub](https://img.shields.io/badge/GitHub-black?logo=github)
![Postman](https://img.shields.io/badge/Postman-black?logo=postman)
![Figma](https://img.shields.io/badge/Figma-black?logo=figma)

---

## 🌐 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-%23000000?style=flat-square&logo=linkedin&logoColor=0A66C2)](https://www.linkedin.com/in/muhammad-arslan-a21139202/)
[![Email](https://img.shields.io/badge/Email-%23000000?style=flat-square&logo=gmail&logoColor=EA4335)](mailto:whynot6108@gmail.com)
[![Fiverr](https://img.shields.io/badge/Fiverr-%23000000?style=flat-square&logo=fiverr&logoColor=1DBF73)](https://www.fiverr.com/s/WEqyQw7)
[![Upwork](https://img.shields.io/badge/Upwork-%23000000?style=flat-square&logo=upwork&logoColor=6FBD44)](https://www.upwork.com/freelancers/~015d6833648d77745f?mp_source=share)

---

✨ *Building production-ready mobile apps with clean UI, solid architecture, and real-world integrations.*
